...
def crud_router_factory(
    ...
    @router.get("/", response_model=List[modelResponse], summary=f"Retrieve all {model_name} items", 
                description=f"Fetch a list of all {model_name} records from the database.")
    async def read_all(db: AsyncSession = Depends(deps.get_session), _current_user: User = Depends(deps.get_current_user)):
        """
    ...
    async def read_one(item_id: str, db: AsyncSession = Depends(deps.get_session), _current_user: User = Depends(deps.get_current_user)):
        """
    ...
    async def create_one(item: modelRequest, db: AsyncSession = Depends(deps.get_session), _current_user: User = Depends(deps.get_current_user)):
        """
    ...
    @router.put("/{item_id}", response_model=modelResponse,
                ...
                description=f"🤖 Modify the {model_name} identified by the given ID.")
    async def update_one(item_id: int, item: modelResponse, db: AsyncSession = Depends(deps.get_session), _current_user: User = Depends(deps.get_current_user)):
        """
    ...
    async def delete_one(item_id: int, db: AsyncSession = Depends(deps.get_session), _current_user: User = Depends(deps.get_current_user)):
        """
